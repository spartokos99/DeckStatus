...
withBrowser((req,res,url)=>{
  if(url.pathname==='/api/app'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({version:'2.0.2',mode:screenshotMode,user:demoUser,canControl:true,capabilities:{dashboard:true,history:true,deckOverlays:true,masterOverlay:true,audioWaveform:true,rekordboxSetup:screenshotMode==='rekordbox',prolinkSetup:screenshotMode==='prolink',networkSettings:true,scenes:true,admin:true}}));return true;}
