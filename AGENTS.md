...
- The updater stages packages separately, preserves private data and requires explicit administrator confirmation before shutdown/replacement. Keep its helper in the portable package and exclude `DeckStatus.update` jobs/backups. Test installs only against disposable owned demo copies; see `docs/updater.md`.
...
- Preserve persistent users, ratings, presets, scenes and keys. Scene layers inserted from presets retain a presetId and follow saved design updates; placement, visibility and scene keys stay unchanged. Explicitly detached layers remain independent. Older layers auto-link only when an unchanged named copy matches exactly one preset. Playing history is session-local. Loading a preset or scene must not start audio capture.
